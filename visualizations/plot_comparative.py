...
def plot_client_vs_global_accuracy(
    ...
    if 'test_accuracies' in qfl_results and 'client_losses' in qfl_results:
        ...
        for client_id in range(num_clients):
            ...
            c_acc = [1.0 / (1.0 + l) if not np.isnan(l) else np.nan for l in c_losses]
...
def plot_client_accuracy_variance(
    ...
    bars1 = ax.bar(x - width/2, q_client_acc, width, 
                   label='Quantum FL', color='mediumpurple', edgecolor='black', linewidth=1.5)
    bars2 = ax.bar(x + width/2, c_client_acc, width, 
...
def plot_quantum_advantage_metrics(
    ...
    if len(q_acc) > 0 and len(c_acc) > 0:
        ...
        rounds = np.arange(min_len)
        colors_line = ['red' if i < 0 else 'green' for i in improvement]
...
def generate_all_comparative_plots(
    ...
    try:
        ...
        print("  ✓ Client Parameter Distance Heatmap")
    except Exception as e:
        print(f"  ✗ Failed: {e}")
    ...
    try:
        saved_plots['client_accuracy_variance'] = plot_client_accuracy_variance(
            qfl_results, cfl_results, client_data, save_dir)
        print("  ✓ Client Accuracy Variance (Fairness)")
    except Exception as e:
        print(f"  ✗ Failed: {e}")
    ...
    print("\n" + "="*70)
    print(f"COMPARATIVE ANALYSIS COMPLETE: {len(saved_plots)} plots generated")
    print("="*70 + "\n")
...
if __name__ == "__main__":
    print("Comparative Analysis Visualization Module")
    print("Import this module and call generate_all_comparative_plots()")
    print("\nExample usage:")
    print("  from viz_comparative_analysis import generate_all_comparative_plots")
    print("  plots = generate_all_comparative_plots(qfl_results, cfl_results, ...)")