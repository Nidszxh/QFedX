...
def create_plots_folder(base_path: str = "./results/cfl") -> Path:
    """Create and return plots folder path."""
...
def plot_client_training_losses(client_losses_per_round: list[list[float]],
    ...
    if save_path is None:
        save_path = create_plots_folder() / "client_training_losses.png"

    # Transpose to get per-client history
    client_losses_dict: dict[int, list[tuple[int, float]]] = defaultdict(list)
    ...
    plt.close(fig)
    logger.info("Client training losses plot saved: %s", save_path)
...
def plot_client_final_accuracies(client_data: list[tuple[torch.Tensor, torch.Tensor]],
    ...
    """
    if save_path is None:
        save_path = create_plots_folder() / "client_final_accuracies.png"
    ...
    plt.savefig(save_path, dpi=150, bbox_inches='tight')
    plt.close(fig)
    logger.info("Client final accuracies plot saved: %s", save_path)
...
def plot_client_label_distribution_heatmap(client_data: list[tuple[torch.Tensor, torch.Tensor]],
    ...
    """
    if save_path is None:
        save_path = create_plots_folder() / "client_label_distribution_heatmap.png"
    ...
    sns.heatmap(label_counts, annot=True, fmt='.0f', cmap='YlGnBu',
    ...
    plt.savefig(save_path, dpi=150, bbox_inches='tight')
    plt.close(fig)
    logger.info("Client label distribution heatmap saved: %s", save_path)
...
def plot_training_dynamics(metrics: dict, save_path: Optional[str] = None):
    ...
    """
    if save_path is None:
        save_path = create_plots_folder() / "training_dynamics.png"
    ...
    plt.savefig(save_path, dpi=150, bbox_inches='tight')
    plt.close(fig)
    logger.info("Training dynamics plot saved: %s", save_path)
...
def plot_confusion_matrix(model: nn.Module, test_data: tuple[torch.Tensor, torch.Tensor],
    ...
    """
    if save_path is None:
        save_path = create_plots_folder() / "confusion_matrix.png"
...
def plot_metric_correlation(metrics: dict, save_path: Optional[str] = None):
    ...
    """
    if save_path is None:
        save_path = create_plots_folder() / "metric_correlation.png"
    ...
    plt.savefig(save_path, dpi=150, bbox_inches='tight')
    plt.close(fig)
    logger.info("Metric correlation heatmap saved: %s", save_path)
...
def plot_conv_feature_maps(model: nn.Module, sample_input: torch.Tensor,
    ...
    """
    if save_path is None:
        save_path = create_plots_folder() / "conv_feature_maps.png"
    ...
    plt.savefig(save_path, dpi=150, bbox_inches='tight')
    plt.close(fig)
    logger.info("Feature maps visualization saved: %s", save_path)
...
def plot_embeddings(model: nn.Module, test_data: tuple[torch.Tensor, torch.Tensor],
    ...
    """
    if save_path is None:
        save_path = create_plots_folder() / f"embeddings_{method}.png"
    ...
    plt.savefig(save_path, dpi=150, bbox_inches='tight')
    plt.close(fig)
    logger.info("Embeddings visualization saved: %s", save_path)
...
def create_summary_table(metrics: dict, config: dict, save_path: Optional[str] = None):
    ...
    """
    if save_path is None:
        save_path = create_plots_folder() / "summary_metrics.csv"
    ...
    logger.info("Summary table saved: %s", save_path)
    ...
    plt.close(fig)
    logger.info("Summary table image saved: %s", table_img_path)
...
def generate_all_cfl_visualizations(
    ...
    """
    logger.info("GENERATING CLASSICAL FL VISUALIZATIONS")

    create_plots_folder(save_dir)
    ...
    logger.info("ALL CLASSICAL FL VISUALIZATIONS COMPLETE")
