...
from core.plot_utils import save_figure, setup_plot_style
...
def plot_global_convergence(test_accuracies: list[float],
    ...
    plt.tight_layout()
    return save_figure(fig, save_dir, filename)
...
def plot_client_boxplot(client_losses: list[list[float]],
    ...
    valid_losses = [losses for losses in client_losses
                   if losses and all(loss_val != float('inf') for loss_val in losses)]

    if not valid_losses:
        logger.warning("No valid client losses for boxplot")
        return ""
...
def plot_round_comparison_dashboard(test_accuracies: list[float],
    ...
    if len(test_accuracies) > 1:
        ...
        ax2.text(0.5, 0.6, 'Best Round', ha='center', fontsize=14, fontweight='bold')
...
def generate_results_table(results: dict,
    ...
    if 'train_losses' in results and results['train_losses']:
        avg_train_loss = np.mean([loss_val for loss_val in results['train_losses'] if loss_val != 0 and loss_val != float('inf')])
...
def generate_all_qfl_plots(results: dict,
    ...
    try:
        ...
        logger.warning(f"Some plots could not be generated: {e}", exc_info=True)
