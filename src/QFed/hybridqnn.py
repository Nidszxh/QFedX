...
class HybridQNN(nn.Module):
    """
    Hybrid QNN with hybrid architecture following best practices:
    ...
    def __init__(self, 
        ...
        # Initialize PennyLane device with fallback
        try:
            self.dev = qml.device("lightning.gpu" if torch.cuda.is_available() else "lightning.qubit", 
                                wires=n_qubits, shots=shots)
        except:
            # Fallback to default simulator
            self.dev = qml.device("default.qubit", wires=n_qubits, shots=shots)
            print("Warning: Using default.qubit simulator as fallback")
    ...
    def _build_qnode(self):
        ...
        # Choose diff_method based on shots and availability
        if self.shots is None:
            diff_method = 'backprop'  # More efficient for statevector simulation
        else:
            diff_method = 'parameter-shift'
        
        @qml.qnode(self.dev, interface='torch', diff_method=diff_method)
        def circuit(inputs, qparams):
            # inputs: shape (n_qubits,) for angle or (2^n_qubits,) for amplitude
            ...
            if self.encoding == 'angle':
                ...
                # Ensure inputs are normalized for amplitude embedding
                inputs_normalized = inputs / (torch.norm(inputs) + 1e-8)
                qml.AmplitudeEmbedding(inputs_normalized, wires=range(self.n_qubits), normalize=True)
    ...
    def forward(self, x):
        ...
        # 2. Quantum processing with improved batching
        q_outputs = []
        ...
        for i in range(batch_size):
            ...
            try:
                ...
                # Convert to tensor while preserving gradients
                if isinstance(single_out, (list, tuple)):
                    single_out = torch.stack([torch.as_tensor(val, dtype=torch.float32, device=device) 
                                            for val in single_out])
                else:
                    single_out = torch.as_tensor(single_out, dtype=torch.float32, device=device)

                # Ensure proper shape
                if single_out.dim() == 0:
                    single_out = single_out.unsqueeze(0)
                    
                q_outputs.append(single_out)
                
            except Exception as e:
                print(f"Warning: Quantum circuit failed for sample {i}: {e}")
                # Fallback: use zero output
                fallback_out = torch.zeros(self.n_readout, dtype=torch.float32, device=device)
                q_outputs.append(fallback_out)

        # Stack quantum outputs
        if q_outputs:
            q_out = torch.stack(q_outputs, dim=0)  # shape (batch_size, n_readout)
        else:
            # Complete fallback if all quantum circuits failed
            q_out = torch.zeros(batch_size, self.n_readout, dtype=torch.float32, device=device)
...
def train_Hybrid_qnn(model, train_loader, val_loader=None, 
...
def main():
    ...
    config = {
        ...
        'shots': None,  # Use None for exact gradients
    }
    
    print("Hybrid Quantum Neural Network Training")
    ...
    # Train the model
    try:
        train_losses, val_accuracies = train_Hybrid_qnn(
            model=model,
        ...
        torch.save({
            ...
            'model_class': 'HybridQNN'
        }, 'Hybrid_qnn_model.pt')

        print("Model saved as 'Hybrid_qnn_model.pt'")
